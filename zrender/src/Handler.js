    ...
    function (require) {
        ...
        var domHandlers = {
            ...
            click: function (event) {
                ...
                // 找到 最后一个hover图形元素，如果没找到，或者找到了可以点击，才会分发click事件
            ...
            mousemove: function (event) {
                ...
                //console.log(this._hasfound);

                // 找到的在迭代函数里做了处理，没找到得在迭代完后处理
                if (!this._hasfound) {
                    if (this._draggingTarget && this._draggingTarget == this._lastHover) {
                        debugger;

                        /**
                         * 在拖动中，是找不到最后一个hover图形元素的，所以程序会走到这里
                         */
                    }
                ...
                /**
                 * 由这段可见，拖动的时候图形元素时最后的addhover，此时真正的_draggingTarget处理不可见状态
                 */
                //如果存在拖拽中元素，被拖拽的图形元素最后addHover
                if (this._draggingTarget) {

                    // 漂移_draggingTarget的位置
                    this.storage.drift(
                ...
                if (this._draggingTarget || this._hasfound || this.storage.hasHoverShape()) {
                    this.painter.refreshHover(); //刷新hover
                }
            },
            ...
            mouseup:function (event) {
                event = this._zrenderEventFixed(event);
                this.root.style.cursor = 'default';
                this._isMouseDown = 0; //设置mousedown为0，不再跟随
                this._mouseDownTarget = null;

                //分发config.EVENT.MOUSEUP事件
                this._dispatchAgency(this._lastHover, EVENT.MOUSEUP, event); //触发zr.on('mouseup',function () {....});
                this._processDrop(event);
                this._processDragEnd(event);
            },
        ...
        Handler.prototype._processDragStart = function (event) {
            ...
            if (this._isMouseDown //键盘已经按下！
                && _lastHover // 最后一个hover图形元素
                && _lastHover.draggable // 最后一个hover图形元素必须是可以拖动的
                && !this._draggingTarget // 因为之后会为这个shape设置一个_draggingTarget，所以不能设置两遍
                && this._mouseDownTarget == _lastHover //并且_lastHover 就是 _mouseDownTarget
            ) {
                ...
                this._isDragging = 1; // 标识：正在被拖动呢

                _draggingTarget.invisible = true; // 标识： 当前shape不可见，由此可见，dnd的时候，拖动的是hover层
                this.storage.mod(_draggingTarget.id, _draggingTarget); //修改可见性
        ...
        Handler.prototype._processDrop = function (event) {
            if (this._draggingTarget) {
                this._draggingTarget.invisible = false; //将真正的_draggingTarget显示出来
                this.storage.mod(this._draggingTarget.id, this._draggingTarget); //设置
                this.painter.refresh(); //刷新
        ...
        Handler.prototype._dispatchAgency = function (targetShape, eventName, event, draggedShape) {
             ...
             * zr.addShape(new CircleShape({
                    ...
                    onclick: function () {
                        return true; //不进行下面的原生转发 zr.on('click',function () {alert('abc'); //这里肯定不会执行！！！});
                    },
            ...
            if (targetShape) {
                ...
                // if (!circleShape.onclick || !circleShape.onclick(eventPacket))
                // 如果onclick return true,那么，就不进行下面的原生转发
        ...
        function findHover(shape) {
            if (
                ...
                //迭代到当前拖拽的图形上
                //这里说明：如果正在拖动当前shape的时候，这个findHover就return false
            ...
            /**
             * 如果鼠标指针进入了当前图形中，就中断迭代查询
             * 1.先判断当前图形是不是个高亮图层，如果是，加入到hover层中
             * 2.如果 当前图形元素不等于 最后一个hover图形元素，那么
             *      2.1 触发shape的mouseout
             *      2.2 触发shape的dragleave
             *      2.3 将当前图形元素指定为 最后一个hover图形元素
             *      2.4 触发shape的dragEnter
             * 3.触发shape的over和dragover
             * 4.设置hasFound为1
             */
            if (shape.isCover(this._mouseX, this._mouseY)) {
                if (shape.hoverable) { //如果当前shape是个高亮的shape，放入到model中
